...
class JadwalmhsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('mhs.jadwal.index',compact('schedule','praktek'));
    }
    ...
    $tgl=date('Y-m-d');
        ...
        if($cek>0){
            ...
            $absen_cek      = Absen_ajar_praktek::where(['kel_praktek'=>$exp[4],'kd_mtk'=>$exp[0]])
                                                     ->orderByDesc('pertemuan');;
            $durasi     = Jadwal::where($w_cek)
                                ->where('selesai','>=', $jam)->count();
            $absen_mhs  = Absen_mhs::Where(['kel_praktek'=>$exp[4],'kd_mtk'=>$exp[0],'pertemuan'=>$absen_cek->first()->pertemuan,'nim'=>Auth::user()->username])->count();
        }else{
           ...
           $absen_cek      = Absen_ajar::where(['kd_lokal'=>$exp[4],'kd_mtk'=>$exp[0]])
                                            ->orderByDesc('pertemuan');
           if($cek_teori>0){
                $durasi     = Jadwal::where(['kd_lokal'=>$exp[4],'kd_mtk'=>$exp[0]])
                                    ->where('selesai','>=', $jam)->count();
                $absen_mhs  = Absen_mhs::Where(['kd_lokal'=>$exp[4],'kd_mtk'=>$exp[0],'pertemuan'=>$absen_cek->first()->pertemuan,'nim'=>Auth::user()->username])->count();

            }else{
                $durasi     = Jadwal::where(['kd_gabung'=>$exp[4],'kd_mtk'=>$exp[0]])
                                    ->where('selesai','>=', $jam)->count();
                $absen_mhs  = Absen_mhs::Where(['kd_gabung'=>$exp[4],'kd_mtk'=>$exp[0],'pertemuan'=>$absen_cek->first()->pertemuan,'nim'=>Auth::user()->username])->count();
